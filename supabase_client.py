...
# GPT tokeniza ~1 token = 4 chars em inglês, ~3 chars em português
# Português tem palavras mais longas, acentos contam como tokens extras
CHARS_PER_TOKEN_PT = 3.2

def estimar_tokens(text: str) -> int:
    """Estima tokens OpenAI a partir de texto em português."""
    if not text:
        return 0
    return max(int(len(text) / CHARS_PER_TOKEN_PT), 10)
...
class SupabaseClient:
    ...
    def _rpc(self, fn, params):
        ...
        if not r.ok:
            raise SupabaseError(f"RPC {fn}: {r.status_code} {r.text[:300]}")
        return r.json()
    ...
    def get_assinatura(self, user_id):
        rows = self.select("assinaturas", filters={
    ...
    def registrar_uso(self, user_id, tokens_input, tokens_output, conversa_id=None, modelo=None, endpoint="chat"):
        ...
        if conversa_id:
            data["conversa_id"] = conversa_id
        try:
            self.insert("uso_tokens", data, returning="minimal")
        except SupabaseError as e:
            log.warning(f"registrar_uso: {e}")
    ...
    def listar_clientes(self, limit=100, offset=0):
        return self.select("v_clientes", filters={
...
def get_user_id_from_token(token: str) -> Optional[str]:
    ...
    # Fallback: decodifica sem verificar (Google OAuth / ES256)
    try:
        import jwt as pyjwt
        payload = pyjwt.decode(
            clean,
...
# ── Cache de assinatura (evita GET repetidos) ───────────────────
_assinatura_cache = {}

def _get_assinatura_cached(user_id: str) -> dict:
    """Busca assinatura com cache simples (1 request por sessão de chat)."""
    import time
    cached = _assinatura_cache.get(user_id)
    if cached and (time.time() - cached["ts"]) < 30:  # cache 30 seg
        return cached["data"]
    ...
    try:
        r = requests.get(
            ...
            params={
                "usuario_id": f"eq.{user_id}",
                "select": "id,tokens_mes,tokens_usados_mes,status",
            },
            headers={
                "apikey": SUPABASE_SERVICE_KEY,
                "Authorization": f"Bearer {SUPABASE_SERVICE_KEY}",
        ...
        _assinatura_cache[user_id] = {"data": result, "ts": time.time()}
        return result
    except Exception as e:
        log.warning(f"[SB] get_assinatura: {e}")
        return {}


def verificar_e_decrementar_tokens(user_id: str, tokens_entrada: int = 0) -> dict:
    """
    Verifica se usuario tem tokens disponíveis.
    Decrementa tokens de entrada (estimado).
    """
    try:
        assin = _get_assinatura_cached(user_id)

        if not assin:
            # Sem assinatura = plano free (limite padrão)
            return {"ok": True}
        ...
        # Estima tokens de entrada em português
        tokens_input_est = estimar_tokens(" " * tokens_entrada) if tokens_entrada > 20 else estimar_tokens("x" * max(tokens_entrada, 100))
        # Mínimo 50 tokens por mensagem
        tokens_input_est = max(tokens_input_est, 50)

        novo_usado = tokens_usados + tokens_input_est
        requests.patch(
            f"{SUPABASE_URL}/rest/v1/assinaturas",
        ...
        # Invalida cache
        _assinatura_cache.pop(user_id, None)

        restantes = tokens_mes - novo_usado
        pct_usado = round((novo_usado / tokens_mes) * 100, 1)
        log.info(f"[tokens] user={user_id[:8]} input=+{tokens_input_est} total={novo_usado}/{tokens_mes} ({pct_usado}%)")
        return {"ok": True, "tokens_restantes": restantes, "pct_usado": pct_usado}

    except Exception as e:
        log.warning(f"[SB] verificar_tokens erro: {e}")
        return {"ok": True}  # Em caso de erro, não bloqueia


def registrar_tokens_resposta(user_id: str, chars_resposta: int = 0):
    """Adiciona tokens da resposta ao consumo do usuario."""
    try:
        tokens_resposta = estimar_tokens("x" * chars_resposta)
        tokens_resposta = max(tokens_resposta, 30)
        ...
        tokens_usados = assin.get("tokens_usados_mes") or 0
        novo_usado = tokens_usados + tokens_resposta
        ...
        pct_usado = round((novo_usado / tokens_mes) * 100, 1)
        log.info(f"[tokens] user={user_id[:8]} output=+{tokens_resposta} total={novo_usado}/{tokens_mes} ({pct_usado}%)")
...
def atualizar_plano_usuario(user_id: str, plano_slug: str, tokens_mes: int = None, payment_id: str = None):
    ...
    try:
        ...
        r2 = requests.patch(
            f"{SUPABASE_URL}/rest/v1/assinaturas",
            params={"usuario_id": f"eq.{user_id}"},
            json=payload,
            headers={
                "apikey": SUPABASE_SERVICE_KEY,
                "Authorization": f"Bearer {SUPABASE_SERVICE_KEY}",
